										...
										<form wire:submit.prevent="save">
												...
												@if ($media_type !== 'none')
														<div class="mb-3">
																...
																@enderror
																@if ($media_file)
																		<small class="text-success d-block mt-1">Baru: {{ $media_file->getClientOriginalName() }}</small>
																@endif
																@if ($oldMediaPath)
																		<small class="text-info d-block mt-1">Saat ini: {{ basename($oldMediaPath) }}</small>
																@endif
														</div>
												@endif
																		...
																		@else
																				...
																				@if (isset($item['media_file']) && $item['media_file'])
																						<small class="text-success d-block mt-1">
																								Baru: {{ $item['media_file']->getClientOriginalName() }}
																						</small>
																				@endif
																				@if (isset($item['media_path']))
																						<small class="text-info d-block mt-1">
																								Saat ini: {{ basename($item['media_path']) }}
																						</small>
																				@endif
																		@endif
																</div>
